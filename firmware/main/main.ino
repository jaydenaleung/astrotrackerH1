...
double raAngle; // current RA angle of the mechanism
bool NORTH_HEM = true;
...
double latAngle; // current latitudinal positioning of mechanism (saved in computer, physically set at start), not goal latitude (given by NEO-8M)
...
double azAngle; // current azithumal positioning (heading) of mechanism (given by magnometer), not goal heading (0 degrees, directly facing north)
...
void loop() {
  ...
  // button logic
  // da da da yet to be added
}

void trackingStart() { // RA motor
  raSD.enable();
  raSD.moveAtVelocity(AOE_RUN_VELOCITY);
}
